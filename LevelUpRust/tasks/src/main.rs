use tasks::travel_planner::shortest_path;
...
fn main() {
    ...
    // let v = Vigenara::new("LEMON");
    // println!("{}", v.encrypt("attackatdawn"));
    // println!("{}", v.decrypt(&v.encrypt("attack at dawn")));
    // let key = "WHYRUST";
    // let v = Vigenara::new(key);
    // let ciphertext = "PVCDJG PAYCMY JR KUC";
    // let plaintext = v.decrypt(&ciphertext);

    // println!("{}", plaintext);

    if let Some(path) = shortest_path(edge_list, 1000, 9000) {
        println!("1000->9000, {:?}  ", path.keys());
    };
}
